...
class EntSrchBuilder(UIBuilder):
    def __init__(self, frame):
        UIBuilder.__init__(self, frame)
        #self._searchUI=Frame(frame, bd=2, relief=RIDGE)
        self.__txtEnterpriseName = Entry(self._searchUI)
        self.__txtTaxIdentificationNumber = Entry(self._searchUI)
    ...
    def addUIControls(self):
        ...
        self.__lblTaxIdentificationNumber=Label(self._searchUI,text="Tax Identification Number")
        self.__lblCountry=Label(self._searchUI,text="Country: ")
        self.__lblAddres=Label(self._searchUI,text="Address: ")

        self.__lblEnterpriseName.grid(row=1,column=1,padx=10,pady=10)
        self.__txtEnterpriseName.grid(row=1,column=2,padx=10,pady=10)
        self.__lblTaxIdentificationNumber.grid(row=2,column=1,padx=10,pady=10)
        self.__txtTaxIdentificationNumber.grid(row=2,column=2,padx=10,pady=10)
        self.__lblCountry.grid(row=3,column=1,padx=10,pady=10)
        self.__txtCountry.grid(row=3,column=2,padx=10,pady=10)
        self.__lblAddres.grid(row=4,column=1,padx=10,pady=10)
        self.__txtAddress.grid(row=4,column=2,padx=10,pady=10)

    def initialize(self):
        self.__txtEnterpriseName.insert(0,"") 
        self.__txtTaxIdentificationNumber.insert(0,"")
        self.__txtCountry.insert(0,"")   
        self.__txtAddress.insert(0,"")

    def getSQL(self):
        return "Select * from Enterprise where EnterpriseName='"+self.__txtEnterpriseName.get()+"' and TaxIdentificationNumber='"+self.__txtTaxIdentificationNumber.get()+"' and Country='"+self.__txtCountry.get()+"'  and Address='"+self.__txtAddress.get()+"'"
#End of class
